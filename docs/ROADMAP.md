...
| 5 | 트레이 메뉴 한국어 하드코딩 | ⚠️ | `src/main/index.ts`에 한국어 직접 입력, i18n 미적용 |
| 6 | DB 레거시 테이블 잔존 | ⚠️ | `review_decisions`, `scan_discoveries` — 실제 DB에 남아있음 |
| 7 | Export 파일 실패 시 로깅 누락 | ⚠️ | 개별 파일 복사 실패 시 기록 안 함 |
