...
| 3 | 파일명 변경 / 날짜별 일괄 정리 | 기능 | 신규, 사진 정리 워크플로우 핵심 니즈 |
...
| # | 항목 | 분류 | 설명 |
|---|------|------|------|
| 1 | Worker Threads (piscina) | 성능 | parallelThreads 설정 미반영, 4~8x 가속 예상 |
...
| # | 항목 | 분류 | 설명 |
|---|------|------|------|
| 1 | 지도 기반 위치 필터링 | 기능 | GPS 좌표 활용, GPS 인덱스 추가 필요 |
| 2 | 메타데이터 편집 (F3) | 기능 | EXIF/XMP 편집 및 동기화 |
| 3 | 동영상 지원 | 기능 | PRD Out of Scope |
| 4 | 클라우드 백업 연동 | 기능 | 선택적 동기화 |
| 5 | SSIM 고해상도 + GPU 가속 | 성능 | 512×512 이상 시 WebGPU 검토 |
| 6 | 대규모 스케일링 500K+ | 아키텍처 | BK-Tree→LSH, 스트리밍 배치 |
| 7 | 스캔 이력/프로젝트 개념 | 아키텍처 | Export 부활 검토 시점 |
